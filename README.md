...
The proxy includes the Responses `tools` list in the prompt sent to Qwen. Codex owns the actual filesystem/command permissions; the model only chooses among tool names the client exposes. If Qwen emits common file-tool aliases like `read_file` while Codex only supports `exec`, the proxy rewrites them to equivalent `exec` shell commands.
